...
def load_methodology_agents_md(context_length: Optional[int] = None) -> str:
    """Load generated profile-local methodology context, if present.

    ``AGENTS.methodology.md`` is generated from accepted methodology distillation
    proposals.  It lives in HERMES_HOME like SOUL.md, not in the project cwd.
    """
    try:
        from hermes_cli.config import ensure_hermes_home
        ensure_hermes_home()
    except Exception as e:
        logger.debug("Could not ensure HERMES_HOME before loading AGENTS.methodology.md: %s", e)
    ...
    try:
        ...
        if not content:
            return ""
        content = _scan_context_content(content, "AGENTS.methodology.md")
        result = (
            "## AGENTS.methodology.md\n\n"
            "The following profile-local methodology notes were generated by Hermes "
            "methodology distillation. They are lower priority than SOUL.md, explicit "
            "user instructions, and manual AGENTS/project context.\n\n"
            f"{content}"
        )
        ...
        logger.debug("Could not read AGENTS.methodology.md from %s: %s", path, e)
        return ""
