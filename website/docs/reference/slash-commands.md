...
| `/distill [history\|pending\|diff\|approve\|reject\|approval]` (aliases: `/methodology`, `/methodology-distillation`) | Review generated methodology proposals. Hermes captures compact session signals, routes candidates as `methodology`, `skill`, `soul`, `docs`, or `drop`, and only stages reusable workflow/judgment rules for `AGENTS.methodology.md`. Use `/distill history --dry-run --profile all` to inspect historical candidates, `/distill history --stage` to create pending methodology proposals, and `/distill pending` / `/distill diff <id>` / `/distill approve <id>` to review them. `SOUL.md` and explicit instructions outrank generated methodology notes. |
...
| `/distill [history\|pending\|approve\|reject\|diff\|approval]` | Review pending methodology writes staged by the `methodology_distillation.write_approval` gate. `/distill history --dry-run --profile all` summarizes historical candidates and their review routes; `/distill history --stage` stages only `methodology` candidates. Full diffs are available with `/distill diff <id>` or in `~/.hermes/pending/methodology_distillation/<id>.json`. |
...
- `/skills` is **CLI-only for search/browse/install**; its write-approval review subcommands (`pending`, `approve`, `reject`, `diff`, `approval`) also work on messaging platforms when `skills.write_approval` is on. `/memory` and `/distill` work on **both** surfaces.
